...
def p_program(p):
        ...
        for child in p[0].children:
            if child.type is 'DECL':
                ...
                function_object = global_symbol_table.getChildFunctionTable(funcname).child_table.table
                func_size = function_object # NEED TO COMPLETE THIS
                f_cfg.write('function ' + funcname + child.params['PARAMS'] + '\n')
                # f_assembly.write(funcname + ':' + '\n')
                # f_assembly.write("# Prologue begins" + '\n')
                # f_assembly.write('\t' + "sw $ra, 0($sp)  # Save the return address" + '\n')
                # f_assembly.write('\t' + "sw $fp, -4($sp)    # Save the frame pointer" + '\n')
                # f_assembly.write('\t' + "sub $fp, $sp, 8    # Update the frame pointer" + '\n')
                # f_assembly.write('\t' + "sub $sp, $sp, %d   # Make space for the locals" + '\n', func_size)
                # f_assembly.write("# Prologue ends" + '\n')
                ...
                # f_assembly.write("# Epilogue begins" + '\n')
                # f_assembly.write('epilogue_' + funcname + ':' + '\n')
                # f_assembly.write('\t' + "add $sp, $sp, %d" + '\n', func_size)
                # f_assembly.write('\t' + "lw $fp, -4($sp)" + '\n')
                # f_assembly.write('\t' + "lw $ra, 0($sp)" + '\n')
                # f_assembly.write('\t' + "jr $ra # Jump back to the called procedure" + '\n')
                # f_assembly.write("# Epilogue ends" + '\n')
