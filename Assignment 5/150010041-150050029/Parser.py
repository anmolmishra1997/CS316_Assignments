...
class symbol_table():
	...
	def printVariables(self):
		for elem in self.table['Entries'].keys():
			print( self.table['Entries'][elem].name )
...
class variable_table_entry():
	def __init__(self,type, name, data_type, pointer_level, width = None, offset = None):
		self.type = type
		self.name = name
		...
		if pointer_level == 0 and data_type == 'float':
			self.width = 8
		# else:
		# 	self.width = 4
		self.offset = 0
...
class function_table_entry():
	def __init__(self, type, name, return_type, pointer_level, parameter_types, child_table):
		...
		self.localsize = 0
		# self.computeOffsets() # added by me
		# if self.localsize == 0:
			# print("LOCAL SIZE OF THE FUNCTION = 0")
	...
	def computeOffsets(self):
		current = 0
		params = 0
		for variable in sorted(self.child_table.table['Entries'].keys()):
			flag = 0
			for parameter in self.parameter_types:
				if variable == parameter['name']:
					flag = 1
					break
			if flag == 1:
				params += self.child_table.table['Entries'][variable].width
				continue
			current += self.child_table.table['Entries'][variable].width
			self.child_table.table['Entries'][variable].offset = current
		self.localsize = current
		# Add offsets for the parameters ** TODO **
		# print(self.name)
		# print(current)
		# print(params)
		subtract = 0
		for parameter in self.parameter_types:
			got_it = False
			for variable in self.child_table.table['Entries'].keys():
				if variable == parameter['name']:
					dict_key = variable
					got_it = True
					break
			subtract += self.child_table.table['Entries'][dict_key].width
			self.child_table.table['Entries'][dict_key].offset = max(current,8) + params + 8 - subtract

	def getParameterOffsets(self):
		list_offsets = []
		for parameter in self.parameter_types:
			...
			for variable in self.child_table.table['Entries'].keys():
				if variable == parameter['name']:
					...
					break
			if not got_it:
				print("MAJOR FUBAR. Did not find parameter.")
			else:
				list_offsets.append( self.child_table.table['Entries'][dict_key].offset )
		# REVERSE THIS LIST
		ret_list = list( reversed(list_offsets) )
		return ret_list
...
def p_program(p):
		...
		f_asm.write('\t' + ".data")
		f_asm.write('\n')
		# ADD information about Global Variables here
		dict_variables = global_symbol_table.table['Entries']
		for var_key in sorted(dict_variables.keys()):
			name = "global_" + str( dict_variables[var_key].name )
			global_type = ""
			if dict_variables[var_key].data_type == 'float' and dict_variables[var_key].pointer_level == 0:
				global_type = ".space " + '\t' + str( dict_variables[var_key].width )
			else:
				global_type = ".word" + '\t' + "0"
			f_asm.write(name + ":" + '\t' + global_type)
			f_asm.write('\n')
		f_asm.write('\n')

		# COMPUTING ALL OFFSETS BEFORE ONLY
		for elem in global_symbol_table.table['FunctionEntries'].keys():
			global_symbol_table.table['FunctionEntries'][elem].computeOffsets()

		for child in p[0].children:
			if child.type is 'DECL':
				pass # since already added information about the global variables
				...
				# global_symbol_table.getChildFunctionTable(funcname).computeOffsets()
				# Conclusion of below - parameters are also there in the variable entries of a symbol table
				# just to see what all is there in the variable table of a function 
				# print(funcname)
				# global_symbol_table.getChildFunctionTable(funcname).child_table.printVariables()

				func_size = global_symbol_table.getChildFunctionTable(funcname).getLocalSize()
				# print(func_size)

				f_asm.write('\t' + ".text" + '\t' + "# The .text assembler directive indicates" + '\n')
				f_asm.write('\t' + ".globl " + funcname + '\t' + "# The following is the code" + '\n')
				f_asm.write(funcname + ':' + '\n')
				f_asm.write("# Prologue begins" + '\n')
				f_asm.write('\t' + "sw $ra, 0($sp)" + '\t' + "# Save the return address" + '\n')
				f_asm.write('\t' + "sw $fp, -4($sp)" + '\t' + "# Save the frame pointer" + '\n')
				f_asm.write('\t' + "sub $fp, $sp, 8" + '\t' + "# Update the frame pointer" + '\n')
				f_asm.write('\t' + "sub $sp, $sp, " + str(8 + func_size) + '\t' + "# Make space for the locals" + '\n')
				f_asm.write("# Prologue ends" + '\n')

				begin = ast_cfg.Block()
				end = ast_cfg.Block()

				begin_asm = ast_cfg.Block()
				end_asm = ast_cfg.Block()
				...
				if isinstance(nodenow, ast_cfg.Node) and nodenow.type is not 'RETURN':
					...
					assembly.assembly_top(nodenow, begin_asm, end_asm, global_symbol_table.getChildFunctionTable(funcname).child_table)
					# passed an instance of class symbol_table
					assembly.label_asm(begin_asm)

					end.id = "<bb " + str(ast_cfg.bid) + ">"
					ast_cfg.bid += 1
					endid_assigned = True

					end_asm.id = "label" + str(assembly.labelid)
					assembly.labelid += 1
					endid_assigned = True
					...
					asm_list = []
					assembly.print_asm(begin_asm, asm_list)
					for elem in asm_list:
						f_asm.write(elem)
				...
				if isinstance(nodenow, ast_cfg.Node) and nodenow.type is 'RETURN' and len(nodenow.children) > 0:
					...
					write_asm = assembly.process_asgn(nodenow.children[0], end_asm, 0)
					for statement in end_asm.statements:
						f_asm.write('\t' + statement + '\n')
					# figure out the move to $v1 in the statements there itself ?
					f_asm.write('\t' + "move $v1, " + write_asm + " # move return value to $v1" + '\n')
					assembly.free_register(write_asm)
					f_asm.write('\t' + "j epilogue_" + funcname + '\n')
				else:
				...
				f_asm.write('\n')
				f_asm.write("# Epilogue begins" + '\n')
				f_asm.write("epilogue_" + funcname + ":" + '\n')
				f_asm.write('\t' + "add $sp, $sp, " + str(8 + func_size) + '\n')
				f_asm.write('\t' + "lw $fp, -4($sp)" + '\n')
				f_asm.write('\t' + "lw $ra, 0($sp)" + '\n')
				f_asm.write('\t' + "jr $ra" + '\t' + "# Jump back to the called procedure" + '\n')
				f_asm.write("# Epilogue ends" + '\n')
				# f_asm.write('\n')
		sym()
...
def p_function_2(p):
		...
		if p[2] != "main":
			...
			# print(p[2])
			a = []
			for child in p[6].children:
				a.append(child)
			newnode = ast_cfg.Node("stmt_list", a, p[2])
			p[0] = ast_cfg.Node("FUNCTION", [newnode], func_name)
		p[0].params['RETURNS'] = p[1]
		p[0].params['PARAMS'] = '(' + ')'
...
def getFunctionParameters(function, arg_list_types):
	...
	while table != None:
		...
		if var:
			params_list_correct_order = deepcopy(var.parameter_types)
			params_list_correct_order.reverse()
			if matchParameterTypes(params_list_correct_order, arg_list_types):
...
if __name__ == "__main__":
	...
	print_f_asm = "Parser_asm_" + fname + ".s"
	f_asm = open(print_f_asm,"w")
	f = open(fname, "r")
	data = f.read()
