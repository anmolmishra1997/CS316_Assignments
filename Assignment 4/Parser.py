...
def p_program(p):
        ...
        ast(p[0], 0)
        sym()
        # print(global_symbol_table.table['FunctionEntries'])
        # for function_name in global_symbol_table.table['FunctionEntries']:
        #     function_object = global_symbol_table.getChildFunctionTable(function_name)
        #     # print(.child_table.table)
        #     # print(function_object.parameter_types)

        # for variable in global_symbol_table.table['Entries']:
        #     variable_object = global_symbol_table.table['Entries'][variable]
        #     f_sym.write(variable_object.name + "\t|\t" + "global" + "\t\t|\t" + variable_object.data_type + "\t|\t" + variable_object.generatePointerString() + '\n')
        # # print(current_symbol_table.table)
        # # begin = Block()
        # # end = Block()
        # # cfg(p[0], begin, end)
...
def p_assign(p):
        ...
        """
        p[0] = Node("ASGN", [p[1], p[3]], p[2], check_indirection_levels(p[1].params, p[3].params))
...
def p_R1_expression(p):
        ...
        if p[2] == '+':
            p[0] = Node("PLUS", [p[1], p[3]], p[2], check_arithmetic_validity(p[1].params, p[3].params))
        elif p[2] == '-':
            p[0] = Node("MINUS", [p[1], p[3]], p[2], check_arithmetic_validity(p[1].params, p[3].params))
        elif p[2] == '*':
            p[0] = Node("MUL", [p[1], p[3]], p[2], check_arithmetic_validity(p[1].params, p[3].params))
        elif p[2] == '/':
            p[0] = Node("DIV", [p[1], p[3]], p[2], check_arithmetic_validity(p[1].params, p[3].params))
        else:
            p[0] = p[2]
...
def p_R1_uminus(p):
        ...
        p[0] = Node("UMINUS", [p[2]], p[1], check_unary_arithmetic_validity(p[2].params))
...
def p_ALLNUM(p):
        ...
        """
        if len(p) > 2:
            p[0] = Node("UMINUS", [p[2]], p[1], p[2].params)
        else:
            p[0] = getPrimitiveParameters(p[1])
...
def p_L2(p):
        ...
        L2 : STAR L2
        """
        p[0] = Node("DEREF", [p[2]], p[1], p[2].params)
...
def p_L2_primitive(p):
        ...
        p[0] = Node("DEREF", [var], p[1], var.params)
        changePointerLevel(p[0], -1, p)
...
def getPrimitiveParameters(primitive):
    ...
    if primitive[0] == 'V':
        ...
        primitive_name = process_core(primitive)

        while table != None:
            var = table.getVariableEntry(primitive_name)
            if var:
                final_obj = Node("primitive", [primitive], primitive_name, {'type' : var.data_type, 'PointerLevel' : var.pointer_level})
                return final_obj
        ...
        table = current_symbol_table
        number = process_core(primitive)

        is_floating = number.find('.')

        if is_floating != -1:
            return Node("primitive", [primitive], number, {'type' : 'float', 'PointerLevel' : 0})
        else:
            return Node("primitive", [primitive], number, {'type' : 'int', 'PointerLevel' : 0})
...
def p_PName(p):
        ...
        if p[1] == '*':
            ...
            p[0] = Node("ADDR", [p[2]], p[1], p[2].params)
            changePointerLevel(p[0], 1, p)
...
def p_PName_Primitives(p):
        ...
        var = getPrimitiveParameters(p[2])
        if p[1] == '*':
            p[0] = Node("DEREF", [p[2]], p[1], var.params)
            changePointerLevel(p[0], -1, p)
        elif p[1] == '&':
            p[0] = Node("ADDR", [p[2]], p[1], var.params)
            changePointerLevel(p[0], 1, p)
...
def p_function_call(p):
        ...
        function_call : NAME LPAREN arg_list RPAREN
                    |   NAME LPAREN RPAREN
        """
        function_name = process_core(p[1])

        if len(p) > 4:
            arg_list_types = p[3].params['type_list']
            params = getFunctionParameters(function_name, arg_list_types)

            p[0] = Node("CALL " + function_name, p[3].children, p[1], params)
        else:
            arg_list_types = []
            params = getFunctionParameters(function_name, arg_list_types)
            p[0] = Node("CALL " + function_name, [], p[1], params)
...
def p_condition(p):
        ...
        """
        if p[2] == '>':
            p[0] = Node("GT", [p[1], p[3]], p[2], check_indirection_levels(p[1].params, p[3].params))
        elif p[2] == '<':
            p[0] = Node("LT", [p[1], p[3]], p[2], check_indirection_levels(p[1].params, p[3].params))
        elif p[2] == '>=':
            p[0] = Node("GE", [p[1], p[3]], p[2], check_indirection_levels(p[1].params, p[3].params))
        elif p[2] == '<=':
            p[0] = Node("LE", [p[1], p[3]], p[2], check_indirection_levels(p[1].params, p[3].params))
        elif p[2] == '==':
            p[0] = Node("EQ", [p[1], p[3]], p[2], check_indirection_levels(p[1].params, p[3].params))
        elif p[2] == '!=':
            p[0] = Node("NE", [p[1], p[3]], p[2], check_indirection_levels(p[1].params, p[3].params))
